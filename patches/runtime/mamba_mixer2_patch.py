...
class MambaMixer2(MambaBase, PluggableLayer):
    ...
    **selective** state spaces)
    ...
    def conv_ssm_forward(
        ...
        # Skip during CUDA graph capture — synchronize() is illegal while capturing.
        # The sync is only needed for eager execution to work around Triton PTX
        # codegen missing memory barriers on SM12x. During graph capture/replay,
        # the CUDA graph runtime handles ordering correctly.
        if self._needs_triton_sync and not torch.cuda.is_current_stream_capturing():
