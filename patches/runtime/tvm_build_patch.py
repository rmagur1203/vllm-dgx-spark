 ...
 private:
  ...
  static int ManagedTensorFromPyObjectNoSync(void* py_obj, DLManagedTensorVersioned** out) {
    try {
      py::handle handle(static_cast<PyObject*>(py_obj));
      at::Tensor tensor = handle.cast<at::Tensor>();
      // Fix for SM121 (GB10): functionalized tensors from PyTorch custom op
      // dispatch may not have storage, causing data_ptr() to throw.
      // Materialize via clone() so toDLPackImpl can safely call data_ptr().
