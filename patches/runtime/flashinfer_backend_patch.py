...
class FlashInferBackend(AttentionBackend):
    ...
    def get_required_kv_cache_layout(cls) -> KVCacheLayoutType | None:
        ...
        # SM100 (Blackwell B-series) and SM120 (Blackwell GB10/DGX Spark)
        # both use TRT-LLM FMHA which requires HND KV cache layout
    ...
    def forward(
        ...
        if num_decode_tokens > 0:
            ...
            if not decode_use_trtllm:
                ...
                # Build causal mask for XQA speculative decoding on SM120.
                # XQA requires a bit-packed uint16 mask when q_len_per_req > 1.
                ...
                if q_len_per_req > 1:
                    ...
                    spec_mask = torch.zeros(
                        (batch_size, q_seq_len, mask_size_per_row),
                        dtype=torch.uint16,
                        device=decode_query.device,
                    )
                    # Build lower-triangular causal mask (bit-packed)
                    for i in range(q_seq_len):
                        # Bits 0..i should be set (token i attends to 0..i)
                        bits = (1 << (i + 1)) - 1
                        spec_mask[:, i, 0] = bits
